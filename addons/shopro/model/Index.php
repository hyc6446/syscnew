...
//        $data['store'] = isset($config['store']) ? json_decode($config['store'], true) : [];
    ...
    {
        $id = $this->request->get('id');
        $data = (new Banner())->field('content,id')->where(['id'=>$id])->find();
        $this->success('',$data);
    }
    ...
    public function news()
    {
        ...
        $data = (new News())->field('title,id')->whereNotNull('deletetime')->paginate($limit,false,['page'=>$page]);
        $this->success('',$data);
    }
    ...
    public function upload()
    {
        ...
        // var_dump($fileInfo['type']);
        // if(in_array($typeArr[1],$mimetypeArr)){
        //     var_dump($typeArr[1]);die;
        //     $suffix = $typeArr[1];
        // }
