...
class UserCollect extends Model
{
        ...
        try {
            ...
            if (isset($id) && $id) {
                ...
                if (!isset($notShard)  && $goods['asset_id'] && $user['addr']) {
                    ...
                    //编号  状态:0=正常,1=正在寄售,3=已合成,

                    // $collectCount = self::where(['status' => ['in', [0, 1, 3]], 'goods_id' => $goods_id])->count();

                    // if ($collectCount>=$goods['issue_num']) {
                    //     $collect->sn = '#'.$goods['issue_num'].'/'.$goods['issue_num'];
                    // }else{
                    //     $collect->sn = '#'.($collectCount+1 >$goods['issue_num']?$goods['issue_num']:$collectCount+1).'/'.$goods['issue_num'];
                    // }
                }
                ...
                //之前的字段
                $collect->querysds = $txRes['data']['type'] ?? '';
                ...
                // exit;
                // } else {
                // $collect = self::setData($collect,$params_data);
                // $collect->save();
                // }
                // var_dump($txRes);
                // exit;

                // $sku = \app\admin\model\shopro\goods\SkuPrice::where('goods_id', $goods_id)->field('sales')->find();
                // $collect->sn = $sku['sales'] + 1;
                $collect->saveAll($collects);
            }
            Db::commit();
            return $collect;
        } catch (\Exception $e) {
    ...
    // public static function setData($collect,$data)
    // {
    //     if (!empty($data)) {
    //         extract($data);
    ...
    //         $collect->owner_addr = $owner_addr ?? ''; //资产账户地址
    //         $collect->status = 0; //状态:0=正常,1=正在寄售,2=已售出,3=已合成,4=已赠予
    //         $collect->from_type = $from_type;
    //         $collect->token = md5($user_id . 'token-' . $referral_code . time());
    //         $collect->up_brand = $brand ? implode('&', $brand) : '-';
    //         $collect->auth_brand = '文昌链Avata'; //授权方
    //         $collect->up_num = $issue_num ?? 1;
    //     }
    //     return $collect;
    // }
